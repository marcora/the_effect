...
p_value <- mean(abs(obs_prop - 0.5) <= abs(null_props - 0.5))
...
## Comparison of Bayesian, likelihood and frequentist inferences when 10/10 flips are point-up in a follow-up experiment
...
```{r}
# Bayesian inference with beta(2, 3) prior
prior_beta <- dbeta(theta_grid, shape1 = 2 + 4, shape2 = 3 + 6)
posterior_unnorm <- prior_beta * likelihood
posterior <- posterior_unnorm / sum(posterior_unnorm)
...
gf_line(density ~ theta, color = ~ fn, data = plot_df) |>
  gf_labs(title = "Bayesian inference with beta(2 + 4, 3 + 6) prior",
          subtitle = "i.e., the posterior from the previous experiment",
          x = expression(theta),
          y = "density")
```
