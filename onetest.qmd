...
![](images/clipboard-3357125979.png)

<https://youtu.be/YKMZIzYBgTk>
...
## Parametric bootstrapping of estimator under the null
...
parm |> summarize(p.value = mean(abs(diff) >= abs(obs_diff))) |> pull(p.value)
```
...
perm <- do(M) * list(diff = diff(mean(Y ~ shuffle(X), data = d)))
```

```{r}
# Compute two-sided p-value
perm |> summarize(p.value = mean(abs(diff) >= abs(obs_diff))) |> pull(p.value)
```
...
perm_t <- do(M) * t.test(Y ~ shuffle(X), data = d, var.equal = TRUE)$statistic
```

```{r}
# Two-sided permutation p-value
perm_t |> summarize(p.value = mean(abs(t) >= abs(obs_t))) |> pull(p.value)
```
...
boot_df <- do(M) * list(boot_diff = diff(mean(Y ~ X, data = resample(d, groups = X))))
```
